...
checkov -d "$TARGET" --framework dockerfile,github_actions,docker_compose,secrets \
  --skip-path '.Codex/worktrees' --skip-path '.venv' --compact --quiet 2>&1 | tail -40
...
trivy fs --scanners vuln,secret,misconfig,license "$TARGET" \
  --severity MEDIUM,HIGH,CRITICAL --skip-dirs '.Codex/worktrees' --skip-dirs '.venv' -q 2>&1 | tail -40
```
...
**Always pass `--skip-dirs/--skip-path .Codex/worktrees` and `.venv`** — those hold dozens of
