...
| Self-elevation grant TTL | 300 s | `security/sentinel/elevation.py:103` (`DEFAULT_SELF_ELEVATION_TTL_SECONDS`) | Bounds a sudo-style re-auth grant (ADR-068 §D) |
| Scoped-2FA grant TTL | 120 s | `security/sentinel/elevation.py:104` (`DEFAULT_SCOPED_2FA_TTL_SECONDS`) | Bounds an agent's owner-signed elevation request |
...
| LLM03 | Supply Chain (skills / MCP / dependencies) | Skill import security scan (`skills/import_pipeline.py`) + signed code-registry entries (`code_registry/verify.py`, ADR-069) + microVM isolation for untrusted code (ADR-093) |
