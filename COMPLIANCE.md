...
| **AT-04** | Resource overload | Quota tracker (`quota/tracker.py`) + per-key rate limiter (`security/rate_limiter.py`) + circuit breakers / retry / fallback (ADR-038, `resilience/`); stage-aware retry caps (`resilience/retry_policy.py`) wired into the node retry loop 2026-08-20 — before that date its cited test exercised an unwired module | `packages/maistro-core/tests/quota/test_tracker.py`; `packages/maistro-core/tests/security/test_rate_limiter.py`; `packages/maistro-core/tests/test_circuit_breaker.py`; `packages/maistro-core/tests/resilience/test_retry_policy.py` | ✅ |
| **AT-05** | Cascading failures | ADR-038 reliability primitives — circuit-breaker state machine + `Fallback[T]` + SLO burn-rate throttling — `resilience/`; in-process rate-limit coordination (`resilience/rate_coordination.py`) wired into node preflight/classification 2026-08-20 — before that date its cited test exercised an unwired module (cross-process file backend still unbuilt) | `packages/maistro-core/tests/resilience/test_fallback.py`; `packages/maistro-core/tests/resilience/test_rate_coordination.py`; `packages/maistro-core/tests/test_circuit_breaker.py` | ✅ |
