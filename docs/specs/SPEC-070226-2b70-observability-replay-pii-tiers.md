...
Replay mode: the container is constructed with `replay_source=trace_id`; the proxies then serve
recorded responses matched by `(seq, request_hash)`. A request whose hash does not match the
...
```sql
ALTER TABLE event_log ADD COLUMN tier VARCHAR(10) NOT NULL DEFAULT 'normal';
ALTER TABLE event_log ADD COLUMN payload_encrypted BYTEA;   -- sensitive tier (KMS)
ALTER TABLE event_log ADD COLUMN payload_hash CHAR(64);     -- secret tier
CREATE TABLE sealed_access_audit (
    id SERIAL PRIMARY KEY,
    event_id INTEGER REFERENCES event_log(id),
    accessor VARCHAR(255), reason TEXT, accessed_at TIMESTAMP DEFAULT NOW()
);
```

- `normal`: `payload` JSONB as today (ADR-037 retention).
- `sensitive`: `payload_encrypted` only; reads require an `observability:sealed` scope and write
  a `sealed_access_audit` row. 30-day retention job.
- `secret`: `payload_hash` + metadata; no payload column populated anywhere.
...
- CI: `grep -rn "litellm.completion\|litellm.acompletion" packages/maistro-core/src` outside
  `observability/proxy/` fails the lint job.

## Acceptance criteria

- [ ] Every LLM/tool call through the proxies writes a `ReplayEvent` carrying the active ADR-037
      `trace_id`/`span_id` and a per-trace monotonic `seq`.
- [ ] `replay(trace_id)` yields events in original `seq` order (property test).
- [ ] Re-running an orchestration path in replay mode never invokes the real LLM or tool
      (asserted via a poisoned real client that raises if touched).
- [ ] A divergent request during replay raises `ReplayDivergenceError` naming the seq and hash diff.
- [ ] `sensitive`-tagged payloads are only readable via the scoped read path; each read writes an
      access-audit row.
- [ ] `secret`-tagged calls persist hash + metadata only — no payload bytes in any table (property
      test over generated payloads).
- [ ] PII detector flags a `normal` event containing an email/card/secret-shaped token: raises in
...
- [ ] Direct `litellm` invocation in engine code fails CI; an agent wired with a non-proxy client
      fails registration.
- [ ] Hot path is never blocked by recording for `normal` tier (buffer-overflow test drops records
