...
|---|---|---|---|
| `[engine-001]` Registry CI tooling | substrate | Accepted; `gap-impl` | Front-matter validator + cross-repo link checker + registry generator + GitHub Action; warn-only → hard fail at day 30 |
| `[engine-002]` INVENTORY auto-regenerated | substrate | Proposed | Once `engine-001` lands, hand-edits to INVENTORY fail CI |
| `[engine-003]` Front-matter on existing engine ADRs | substrate | Accepted; gradual | ADRs 000–029 gain front-matter on touch (not bulk) |
| `[engine-004]` CONTRIBUTING.md and convention docs | substrate | Proposed | Author-facing summary of ADR-031/032/001 |
| `[turing-090]` Front-matter on Turing specs | autonoetic | Proposed; warn-only | 22 top-level + ~70 nested epic stories |
...
| `[conductor-090]` Front-matter on mAIstro specs | conductor | Proposed; warn-only | 91 specs (`S-NNN` → `SPEC-NNN` on touch) |
...
| `[sh-095]` Copier bootstrap | multi-tenant | Proposed; `gap-impl` | Same pattern |
...
| Item | Part | Status | Detail |
|---|---|---|---|
| `[engine-021]` Memory spec dedup | substrate | Accepted; `gap-impl` | Engine ADR-011…017 canonical; product specs become `Substrate:`-cited |
| `[turing-091]` Memory specs `Substrate:` recast | autonoetic | Accepted; `gap-impl` | `turing-dossier`, `turing-memory-consolidator`, `turing-notebook-live-vault`, `turing-obsidian-store` |
| `[conductor-091]` Memory specs `Substrate:` recast | conductor | Proposed; `gap-impl` | `S-008`, `S-009`, `S-032`, `S-033` |
| `[engine-022]` Catalog spec dedup | substrate | Accepted; `gap-impl` | Engine simple form + stronghold multi-tenant variant per `[engine#ADR-035]` |
| `[conductor-092]` Catalog specs `Substrate:` recast | conductor | Proposed; `gap-impl` | `S-005`, `S-138` cite engine ADR-006/009 |
...
| Item | Part | Status | Detail |
|---|---|---|---|
| `[engine-030]` Ontology Semantic facet | substrate | Accepted; `gap-impl` | Per `[engine#ADR-036]`. v1.0 ships Semantic only; Kinetic + Dynamic deferred to v2.0 |
| `[engine-031]` Observability primitives | substrate | Accepted; `gap-impl` | Per `[engine#ADR-037]`. 12 required spans, 6 metrics, 5 event topics |
...
| Item | Status | Detail |
|---|---|---|
| `[conductor-001]` Setup wizard | Proposed | `S-139` is the spec; v1.0 critical path |
| `[conductor-002]` Per-user memory isolation | Proposed | Hard boundary; cross-user retrieval impossible by construction |
| `[conductor-003]` Multi-user auth (Keycloak / JWT) | Proposed | `S-018`, `S-019`, `S-024` |
| `[conductor-004]` Native install + Podman + systemd | Proposed | `S-147`, `S-148` |
| `[conductor-005]` Tailscale-native networking | Proposed | `S-153` |
| `[conductor-006]` Setup-wizard property test | Proposed | A new household can complete setup in < 30 min |
| `[conductor-007]` Per-user isolation property test | Proposed | Cross-user retrieval is structurally impossible |
...
| `[conductor-095]` Adopt contract markers | conductor | Proposed | Same |
...
| `[engine-052]` Compliance gap audit on accepted ADRs | substrate | Proposed | Feeds stronghold COMPLIANCE.md control claims |
...
| `[conductor-100]` Voice + email + Alexa channels | conductor | Proposed | `S-041`…`S-104` |
| `[conductor-101]` Hardware-signing integration | conductor | Proposed | `S-150` (substrate: `engine#ADR-022`) |
...
Phase E.conductor Conductor v1.0     [·] ░░░░░░░░░░   0%   Specs exist; impl pending
Phase E.turing  Autonoetic v1.0      [~] ██░░░░░░░░  20%   Memory tier scaffold + sketches; property tests pending
Phase E.sh      Multi-tenant v1.0    [~] ██████░░░░  60%   Three-boundary scan + memory + RBAC live; multi-tenant + compliance pending
