...
if [[ -n "$GENOME_MODELS" ]]; then
    LIVE_FLAGS="--genome-db /run/reports/population.db --genome-models '$GENOME_MODELS' --roster-size ${MAISTRO_RSI_ROSTER_SIZE:-4} --emergency-models '${MAISTRO_RSI_EMERGENCY_MODELS:-}' --evolve-goal \"\$RSI_GOAL\""
fi
