...
Ported from Stronghold's `agents/request_analyzer.py` (pure `re`/`dataclasses`/
`typing`, no product-specific dependencies).
...
so a follow-up like "yes do it" is sufficient if the prior turn already
proposed a detailed plan the user can confirm.
"""
...
def _is_confirmation(
    ...
    for msg in reversed(conversation_context):
        if msg.get("role") == "assistant":
            ...
            if len(content.split()) >= 15 and _PROPOSAL_PATTERNS.search(content):
                return True
            break
