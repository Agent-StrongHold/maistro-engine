...
logger = logging.getLogger("maistro.skills.canary")
...
class CanaryManager:
    ...
    def start_canary(
        ...
        with self._lock:
            self._deployments[skill_name] = deployment
        logger.info(
    ...
    def get_deployment(self, skill_name: str) -> CanaryDeployment | None:
        """Get active canary deployment."""
        return self._deployments.get(skill_name)

    def should_use_new_version(self, skill_name: str) -> bool:
    ...
    def _advance(self, deployment: CanaryDeployment) -> str:
        ...
        if idx >= len(_STAGE_ORDER) - 1:
            del self._deployments[deployment.skill_name]
