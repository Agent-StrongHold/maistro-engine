"""Agent Catalog — A2A Agent Card registry with cascade resolution.

Agents registered as Agent Cards with version, scope, trust tier,
and priority tier. Cascade resolution: user > builtin.
"""
...
logger = logging.getLogger("maistro.agents.catalog")

_SCOPE_PRIORITY = {"builtin": 0, "user": 2}
...
class AgentCard:
    ...
    @classmethod
    def from_identity(
        cls, identity: AgentIdentity, scope: str = "builtin", user_id: str = ""
    ) -> AgentCard:
        return cls(
            id=identity.name,
            name=identity.name,
...
def _card_visible(card: AgentCard, user_id: str) -> bool:
...
class AgentCatalog:
    """Agent catalog with cascade resolution."""
    ...
    def list_agents(self, user_id: str = "") -> list[AgentCard]:
        seen: dict[str, AgentCard] = {}
        for card in self._cards:
            if not _card_visible(card, user_id):
                continue
