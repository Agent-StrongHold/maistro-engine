...
async def test_full_session_lifecycle_with_safety():
    ...
    # Warden refuses a malicious payload (raises through the wrapper).
    import pytest
...
async def test_policy_engine_gates_actions_per_session():
    # After 0 allowed "rm" actions, the policy denies further ones — the manager's
    # per-session PolicyActionGate must drop them from the harness response.
    ...
    assert tools == ["ls"]  # the rm action was gated out by the sequence policy
