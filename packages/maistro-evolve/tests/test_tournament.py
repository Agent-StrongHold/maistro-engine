...
class TestEloTournament:
    ...
    def test_record_battle_winner_a(self):
        t = EloTournament()
        battle = t.record_battle("ifeval", "g1", "g2", 0.8, 0.4)
    ...
    def test_record_battle_winner_b(self):
        t = EloTournament()
        battle = t.record_battle("ifeval", "g1", "g2", 0.3, 0.9)
        assert battle.winner_id == "g2"

    def test_record_battle_draw(self):
        t = EloTournament()
        battle = t.record_battle("ifeval", "g1", "g2", 0.5, 0.5)
        assert battle.winner_id == "draw"

    def test_elo_updates_on_win(self):
        t = EloTournament()
        t.record_battle("ifeval", "g1", "g2", 0.8, 0.4)
        elo_a = t.get_elo("g1", "ifeval")
        elo_b = t.get_elo("g2", "ifeval")
    ...
    def test_elo_converges_over_many_battles(self):
        t = EloTournament()
        for _ in range(50):
            t.record_battle("ifeval", "strong", "weak", 0.8, 0.2)
        elo_strong = t.get_elo("strong", "ifeval")
        elo_weak = t.get_elo("weak", "ifeval")
    ...
    def test_get_avg_elo(self):
        t = EloTournament()
        t.record_battle("ifeval", "g1", "g2", 0.8, 0.4)
        t.record_battle("bfcl", "g1", "g2", 0.3, 0.7)
    ...
    def test_leaderboard_sorted_by_elo(self):
        ...
        for _ in range(20):
            ...
            t.record_battle("ifeval", "g2", "g3", 0.8, 0.2)
    ...
    def test_leaderboard_filtered_by_benchmark(self):
        ...
        t.record_battle("ifeval", "g1", "g2", 0.8, 0.4)
        t.record_battle("bfcl", "g1", "g2", 0.3, 0.7)
        lb = t.get_leaderboard(benchmark="ifeval")
    ...
    def test_battle_history_filter(self):
        t = EloTournament()
        t.record_battle("ifeval", "g1", "g2", 0.8, 0.4)
        t.record_battle("bfcl", "g1", "g3", 0.5, 0.6)
        history = t.get_battle_history(genome_id="g1")
        assert len(history) == 2
        ife_history = t.get_battle_history(benchmark="ifeval")
        assert len(ife_history) == 1

    def test_stats(self):
        t = EloTournament()
        t.record_battle("ifeval", "g1", "g2", 0.8, 0.4)
        t.record_battle("bfcl", "g1", "g2", 0.5, 0.6)
        stats = t.get_stats()
