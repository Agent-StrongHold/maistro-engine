...
_SCOUT_SYSTEM = (
    ...
    "rounds. You are shown the module, its existing tests, and its uncovered "
    "lines. Identify a RANKED SHORTLIST (1-3) of the highest-impact improvements, "
    "from tightening correctness to ambitious growth, using these kinds:\n"
    ...
    "Favor correctness and verification before growth (do not build v2.0 on "
    "unverified code), but propose bold enhancements when the module is ready. "
    "Rank by impact. Name exactly WHERE (function/test/line) and WHAT, concretely "
    "enough that different implementers could each attempt the same item.\n"
...
def _build_user_prompt(source: str, tests: str, uncovered: Sequence[int] | str) -> str:
    unc = uncovered if isinstance(uncovered, str) else ", ".join(str(n) for n in uncovered)
    return (
        "TARGET MODULE:\n"
        f"{source}\n\n"
        "EXISTING TESTS:\n"
        f"{tests or '(none found)'}\n\n"
        f"UNCOVERED LINES: {unc or '(unknown)'}\n\n"
        "Return the ranked shortlist as the specified JSON array."
    )
...
def scout_shortlist(
    ...
    *,
    ...
    messages = [
        ...
        {"role": "user", "content": _build_user_prompt(source, tests, uncovered)},
    ]
    try:
