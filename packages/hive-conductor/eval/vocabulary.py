"""Declarative check vocabulary for rubric criteria (ADR-060).

The vocabulary is code, audited once. Domains are data (YAML files).
Every op returns a bool: True = criterion passed.
...
Python slice semantics: slice_start defaults to 0 (or None), slice_end to None.
"""
...
# Needed for CTAClarity single_primary_cta: "at most 2 of these CTAs appear".
...
def _eval_count(count: int, check: dict[str, Any]) -> bool:
    mn: int | None = check.get("min")
    mx: int | None = check.get("max")
    return (mn is None or count >= mn) and (mx is None or count <= mx)
...
def _eval_simple(op: str | None, check: dict[str, Any], output: str) -> bool | None:
    """Handle the text-matching and counting ops; return None if op is not handled here."""
    if op == "keywords_any":
    ...
    if op == "word_count":
        return _eval_count(len(output.split()), check)
    return None
