...
PROBLEMS = [
    {"id": "r1", "prompt": "A train travels 60 km/h for 2.5 hours, then 80 km/h for 1.5 hours. What is the total distance?", "answer": 270, "unit": "km"},
    {"id": "r2", "prompt": "If 3x + 7 = 22, what is x?", "answer": 5, "unit": ""},
    {"id": "r3", "prompt": "A store offers 20% off, then an additional 10% off the discounted price. What is the total percentage discount?", "answer": 28, "unit": "%"},
    {"id": "r4", "prompt": "How many ways can you arrange 4 books on a shelf?", "answer": 24, "unit": "ways"},
    {"id": "r5", "prompt": "If a rectangle has perimeter 30 and width 5, what is its area?", "answer": 50, "unit": "sq units"},
]
...
async def run(llm_call: Callable[[str, str], Awaitable[str]], n_problems: int = 5) -> EvalResult:
    ...
    for p in problems:
        ...
        no_contradictions = "but actually" not in response.lower() and "wait, no" not in response.lower()

        score = (60 if correct else 0) + (20 if has_steps else 0) + (20 if no_contradictions else 0)
        total += score
        details["problems"].append({"id": p["id"], "correct": correct, "score": score, "extracted": extracted})
