...
import json, base64, os, subprocess, random, time, re, sys
...
TOP_SITES = ["https://google.com","https://reddit.com","https://amazon.com",
    "https://spotify.com","https://figma.com","https://dribbble.com",
    "https://producthunt.com","https://slack.com","https://discord.com",
    "https://canva.com","https://airtable.com","https://monday.com",
    "https://asana.com","https://miro.com","https://webflow.com"]
...
def screenshot_app():
    with sync_playwright() as p:
        ...
        if s: s.click(); page.wait_for_timeout(300)
...
def get_fix(our_b64, ref_b64s):
    prompt = ("Image 1 is our app. Images 2+ are top sites (the quality bar). "
...
def restart():
    ...
    time.sleep(1)
    subprocess.Popen([str(ROOT/"../../.venv/bin/uvicorn"),"main:app","--host","0.0.0.0","--port",PORT],
...
# Load corpus
refs = [base64.b64encode(f.read_bytes()).decode() for f in CORPUS.glob("*.png") if "holdout" not in f.name and "ours" not in f.name]
...
for i in range(PASSES):
    ...
    if not css:
        print("  ✗ No CSS fix returned"); continue
    # Apply
    ...
    if not build():
        ...
        print("  ✗ Build failed — reverted"); continue
    restart()
    print(f"  ✓ CSS appended + rebuilt + restarted")
