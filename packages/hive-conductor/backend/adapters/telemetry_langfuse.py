...
def _init_tracer():
    ...
    endpoint = os.environ.get("OTEL_EXPORTER_OTLP_ENDPOINT", "") or os.environ.get("LANGFUSE_OTEL_URL", "")
    # Default to Launch service mesh if deployed alongside langfuse-app
    if not endpoint and os.environ.get("STUDIOSHARE_APP_ENV"):
        endpoint = "http://jfc-fuse.launch.internal"
    if not endpoint:
        return None
