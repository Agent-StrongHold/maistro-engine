...
async def run_chat_completion_streaming(
    ...
    ``thinking`` (reasoning token from reasoning-capable models),
    ...
    for iteration in range(5):
        ...
        async for ev in _stream_turn(llm, turn_req, tools_acc, content_parts):
            yield ev
        content_acc = "".join(content_parts)
    ...
    async for ev in _stream_turn(llm, final_req, None, final_parts):
        yield ev
    yield {"type": "done", "content": "".join(final_parts), "model": model}
