...
async def create_project(data: dict) -> dict:
    r = await _req("post", "onboard_projects", json=data,
...
async def list_projects(user_id: str) -> list[dict]:
    r = await _req("get", "onboard_projects",
...
async def get_project(project_id: str) -> dict | None:
    r = await _req("get", "onboard_projects", params={"id": f"eq.{project_id}", "deleted_at": "is.null"})
    r.raise_for_status()
    rows = r.json()
...
async def update_project(project_id: str, data: dict) -> None:
    await _req("patch", "onboard_projects", json=data,
               params={"id": f"eq.{project_id}"})


async def delete_project(project_id: str) -> None:
    from datetime import datetime, timezone
    await update_project(project_id, {"deleted_at": datetime.now(timezone.utc).isoformat()})


# ─── Scans ───

async def create_scan(data: dict) -> dict:
    r = await _req("post", "onboard_scan_results", json=data,
                   headers={"Prefer": "return=representation"})
    r.raise_for_status()
    return r.json()[0]
...
async def get_latest_scan(project_id: str) -> dict | None:
    r = await _req("get", "onboard_scan_results",
...
async def update_scan(scan_id: str, data: dict) -> None:
    await _req("patch", "onboard_scan_results", json=data,
...
async def create_deployment(data: dict) -> dict:
    r = await _req("post", "onboard_deployments", json=data,
                   headers={"Prefer": "return=representation"})
    r.raise_for_status()
    return r.json()[0]
...
async def update_deployment(dep_id: str, data: dict) -> None:
    await _req("patch", "onboard_deployments", json=data,
