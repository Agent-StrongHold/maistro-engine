...
def _config_writer(task_id: str) -> TestClient:
    ...
    stores.users[uid] = stores.users._model_class(
        id=uid, username=uid, password_hash=hash_password("pw"),
        role="user", is_active=True, permissions=["config.write"], created_at=datetime.now(UTC),
    )
    c = TestClient(app)
    assert c.post("/v1/auth/login", json={"username": uid, "password": "pw"}).status_code == 200
    e = c.post("/v1/auth/elevate", json={"password": "pw", "permissions": ["config.write"], "task_id": task_id})
