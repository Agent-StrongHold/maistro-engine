...
async def test_context_forwarded_as_additional_context() -> None:
    stub_root = _make_deepeval_stub(score=0.8, success=True, reason="ok")
    try:
        from eval.deepeval_scorer import DeepEvalScorer
        ...
        call_kwargs = metric_instance.a_measure.call_args
        additional = (
            call_kwargs.kwargs.get("_additional_context") or call_kwargs.args[1]
