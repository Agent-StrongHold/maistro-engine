...
async def run():
    ...
    try:
        ...
        # ─── Step 2: Verify creds are already configured ───
        ...
        # ─── Step 3: browser-use navigates real Jira (READ-ONLY) ───
        ...
        try:
            if "[" in result:
                ...
                jira_issues = json.loads(result[start:end])
        except (json.JSONDecodeError, ValueError):
            pass
        print(f"   ✅ {len(jira_issues)} issues extracted")

        # ─── Step 4: Read issue detail (READ-ONLY) ───
        if jira_issues:
        ...
        # ─── Step 6: Create + run DAG with real context ───
