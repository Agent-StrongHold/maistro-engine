...
async def score_response(question, response):
    ...
    messages = [
        {
            "role": "system",
            "content": 'Score this chatbot response 1-10 on: accuracy, completeness, actionability. Return JSON: {"accuracy": N, "completeness": N, "actionability": N, "total": N}',
        },
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        r = await client.post(
            ...
            headers={
                "Authorization": "Bearer sk-NFdRTYsn3lWfIsJZBhhdbA",
...
async def main():
    ...
    for r in sorted(
        results, key=lambda x: x["avg_score"] / MODEL_COST.get(x["model"], 1.0), reverse=True
    ):
        cost = MODEL_COST.get(r["model"], 1.0)
        efficiency = r["avg_score"] / cost
        print(
            f"  {r['config']:30s} | Score: {r['avg_score']:5.1f} | Latency: {r['avg_latency_s']:5.1f}s | Cost: ${cost} | Efficiency: {efficiency:.1f}"
        )
