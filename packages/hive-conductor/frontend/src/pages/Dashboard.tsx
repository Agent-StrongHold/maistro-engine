      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        {entries.map(([name, count]) => (
          <div key={name} style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ flex: 1, height: 14, background: "var(--rule)", borderRadius: 4, overflow: "hidden" }}>
...
function UnknownWidget({ widget }: { widget: Widget }) {
  ...
  if (cfg.display === "count") {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", height: "100%" }}>
        ...
        {cfg.sub && <div style={{ fontSize: "0.6rem", color: "var(--muted)", marginTop: 4 }}>{cfg.sub}</div>}
      </div>
    );
  }
  ...
  if (cfg.display === "list" && data.records) {
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 2, overflowY: "auto", maxHeight: 240 }}>
        {data.records.length === 0 && <div style={{ fontSize: "0.65rem", color: "var(--muted)" }}>No records</div>}
        {data.records.map((r: { name?: string; id?: string }, i: number) => (
          <div key={i} style={{ fontSize: "0.68rem", padding: "4px 0", borderBottom: "1px solid var(--rule)", color: "var(--pencil)" }}>
            {r.name || r.id || "(untitled)"}
          </div>
        ))}
        <div style={{ fontSize: "0.55rem", color: "var(--muted)", marginTop: 4 }}>{data.records.length} records</div>
      </div>
    );
  }
      ...
      <div style={{ fontSize: "0.65rem", display: "flex", flexDirection: "column", height: "100%" }}>
        <div style={{ overflowX: "auto", overflowY: "auto", flex: 1 }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: cols.length * 100 }}>
            <thead>
              <tr style={{ position: "sticky", top: 0, background: "var(--panel-alt, #F8F7FF)", zIndex: 1 }}>
                {cols.map(c => (
                  <th key={c} style={{ textAlign: "left", padding: "6px 8px", borderBottom: "2px solid rgba(99,102,241,0.3)", color: "var(--accent)", fontSize: "0.58rem", fontWeight: 700, whiteSpace: "nowrap", textTransform: "uppercase", letterSpacing: "0.05em" }}>{c}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.slice(0, 50).map((row, i) => (
                <tr key={i} style={{ background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.02)" }}>
                  {cols.map(c => (
                    <td key={c} style={{ padding: "4px 8px", color: "var(--pencil)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 180, borderBottom: "1px solid rgba(255,255,255,0.03)" }}>{(row[c] || "—").slice(0, 60)}</td>
                  ))}
                </tr>
              ))}
            </tbody>
        ...
        <div style={{ fontSize: "0.55rem", color: "var(--muted)", marginTop: 4 }}>{data.count} rows × {cols.length} columns</div>
      </div>
    );
  }
  ...
  if (data.breakdown && typeof data.breakdown === "object") {
    ...
    const palette = ({
      ...
      royal: ["#7c3aed", "#8b5cf6", "var(--accent)", "#c4b5fd", "#b45309", "#d97706", "#f59e0b", "#fbbf24", "#6d28d9", "#5b21b6", "#4c1d95", "#ddd6fe"],
    ...
    } as Record<string, string[]>)[cfg.theme || "default"] || ["var(--accent)", "#8b5cf6", "#ec4899", "#f43f5e", "#f97316", "#eab308", "#22c55e", "#14b8a6", "#06b6d4", "#3b82f6"];
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {entries.slice(0, 10).map(([label, count], i) => (
            <div key={label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "4px 0" }}>
              <span style={{ width: 20, height: 20, borderRadius: "50%", background: i < 3 ? palette[i] : "var(--rule)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.55rem", fontWeight: 700, color: i < 3 ? "#fff" : "var(--muted)", flexShrink: 0 }}>{i + 1}</span>
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        {data.total && <div style={{ fontSize: "0.55rem", color: "var(--muted)", textAlign: "right", marginTop: 2 }}>{data.total} total</div>}
      </div>
    );
  }
  ...
  if (data.records) {
    return (
      <div style={{ fontSize: "0.7rem" }}>
        <div style={{ color: "var(--muted)", marginBottom: 6, fontSize: "0.58rem", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em" }}>{data.count || data.records.length} records</div>
        {data.records.slice(0, 12).map((rec: any, i: number) => {
          ...
          return (
            <div key={i} style={{ display: "flex", gap: 8, padding: "6px 4px", borderBottom: "1px solid var(--rule)", alignItems: "center" }}>
              ...
              {rec.status && <span style={{ color: "var(--accent)", flexShrink: 0, fontSize: "0.56rem", padding: "2px 8px", borderRadius: 6, background: "rgba(167,139,250,0.1)", fontWeight: 600 }}>{rec.status}</span>}
            </div>
          );
        })}
      </div>
...
function AirtableCascade({ cfgTable, setCfgTable, cfgGroupBy, setCfgGroupBy, cfgDisplayField, setCfgDisplayField, cfgFilter, setCfgFilter, cfgMaxRecords, setCfgMaxRecords, cfgFields }: any) {
  ...
  return (<>
    ...
    {cfgTable && (<>
      <label style={{ fontSize: "0.58rem", color: "var(--muted)" }}>Group By (chart column)</label>
  ...
  return (
    <div style={{ gridColumn: "span 1", position: "relative" }}>
      ...
      {open && !browsing && (
        ...
        <div style={{ position: "absolute", zIndex: 40, marginTop: 4, background: "var(--panel-bg)", border: `1px solid ${C.border}`, borderRadius: 8, padding: 6, width: 180, boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
          {CATALOG.map(c => (
      ...
      {open && browsing && (
        ...
        <div style={{ position: "absolute", zIndex: 40, marginTop: 4, background: "var(--panel-bg)", border: `1px solid ${C.border}`, borderRadius: 12, padding: 10, width: 360, maxHeight: 420, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
