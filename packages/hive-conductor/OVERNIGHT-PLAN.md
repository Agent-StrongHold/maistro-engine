...
- [ ] **Jira drafts refinement flow** — Chat creates pre-filled draft → guided interview to refine → template with actual Jira fields (required enforced, optional guided) → confirm posts to Jira. **Source material already exists at:** `/Users/blake.matthews.-nd/Library/CloudStorage/OneDrive-TheWaltDisneyCompany/Documents/dev/Automate Airtable to Jira/` — includes `epic_template.md`, `jira_crafting.md`, `jedai-pm-knowledge-base-personas.md`, `base_context.md`, and the full `mcp-atlassian` client.
