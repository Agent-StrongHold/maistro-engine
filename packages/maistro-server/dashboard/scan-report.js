...
var _FINDING_DETAILS = {
  'code_execution': {
    ...
    mitigation: 'Maistro strips all code execution calls and replaces them with safe placeholders. Skills should use the approved tool dispatch system, never raw code execution.',
  },
  'credential_leak': {
    ...
    mitigation: 'Maistro replaces hardcoded credentials with environment variable placeholders. Legitimate skills should reference credentials via auth_key_env in the YAML frontmatter.',
  },
  'prompt_injection': {
    ...
    mitigation: 'Maistro\'s Warden uses 4-layer detection (regex, heuristics, semantic analysis, AI classifier) to catch injection attempts. The fixer strips known injection phrases, but heavily injected content is marked as "Deeply Flawed" since the core intent is malicious.',
  },
  'Direct instruction override': {
    ...
    mitigation: 'Maistro\'s Warden Layer 2 (heuristic) and Layer 3 (semantic) both detect instruction override patterns. The detection is Unicode-aware (catches Cyrillic lookalikes and RTL marker obfuscation).',
  },
  'Jailbreak (restriction removal)': {
    ...
    mitigation: 'Maistro detects jailbreak patterns across all 4 Warden layers. The fine-tuned blue-team model (Enterprise tier) provides additional detection of novel jailbreak techniques not covered by pattern matching.',
  },
  'Role reassignment': {
    ...
    mitigation: 'Maistro\'s Sentinel validates that tool calls match the agent\'s assigned role and trust tier, regardless of what the prompt says. Even if the prompt claims admin access, Sentinel enforces the actual permissions.',
  },
  'dangerous import': {
    ...
    mitigation: 'Maistro strips dangerous import statements. Skills should use the approved tool dispatch system, which validates each operation through Sentinel before execution.',
  },
  'trust tier': {
    ...
    mitigation: 'Maistro forces all imported skills to T2 (community) regardless of what they claim. Trust tier promotion requires separate AI review + admin approval.',
  },
  'instruction density': {
    ...
    mitigation: 'Maistro marks these as "Deeply Flawed" — they cannot be repaired because there is nothing to repair. The skill is entirely malicious.',
  },
};
...
function _showFindingModal(findingKey) {
  ...
  mitLabel.textContent = 'How Maistro handles this';
