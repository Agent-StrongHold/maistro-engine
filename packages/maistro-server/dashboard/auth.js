/**
 * Maistro Auth Guard (BFF-aware) + Security Violation Popup
 * --------------------------------------------------------------
 ...
 *      Indicator cookie "maistro_logged_in=1" signals active session.
 ...
 *   - authHeaders()       -> {Authorization, Content-Type, X-Maistro-Request}
 *   - maistroLogout()  -> clears session (cookie + localStorage) and redirects
 *   - getMaistroUser() -> display name
 */
(function() {
  'use strict';

  var LOGIN = '/';
  var TOKEN_KEY = 'maistro-token';
  var TOKEN_TYPE_KEY = 'maistro-token-type';
  var USER_KEY = 'maistro-user';
  ...
  var hasCookieSession = document.cookie.split(';').some(function(c) {
    return c.trim().startsWith('maistro_logged_in=');
  });
...
function getAuth() {
  var tokenType = localStorage.getItem('maistro-token-type');
  if (tokenType === 'api_key') {
    var token = localStorage.getItem('maistro-token');
...
function getMaistroUser() {
  return localStorage.getItem('maistro-user') || '';
}

async function parseMaistroResponse(resp) {
...
function maistroErrorMessage(parsed) {
...
function maistroEmptyState(icon, title, desc, ctaText, ctaHref) {
...
(function() {
  ...
  function inject() {
    ...
    logoutBtn.onclick = maistroLogout;
    ...
    _origFetch('/v1/maistro/profile', {headers: authHeaders()}).then(function(r) {
...
(function() {
  ...
  function injectStyles() {
    ...
    style.textContent = [
      ...
      '}',
      '#maistro-strike-modal h2 { font-family:"Playfair Display",Georgia,serif; color:#ff4444; margin:0 0 8px; }',
      '#maistro-strike-modal .sh-strike-badge {',
      ...
      '}',
      '#maistro-strike-modal .sh-body { font-size:0.85rem; line-height:1.6; color:#b0aaa0; }',
      '#maistro-strike-modal textarea {',
      ...
      '}',
      '#maistro-strike-modal textarea:focus { border-color:#e2a529; outline:none; }',
      '#maistro-strike-modal .sh-btn {',
      '  padding:8px 20px; border:none; border-radius:6px; cursor:pointer;',
      '  font-family:"JetBrains Mono",monospace; font-size:0.8rem; font-weight:600;',
      '}',
      '#maistro-strike-modal .sh-btn-appeal { background:linear-gradient(135deg,#c9a227,#e2a529); color:#1a1a2e; }',
      '#maistro-strike-modal .sh-btn-appeal:hover { box-shadow:0 0 12px rgba(226,165,41,0.3); }',
      '#maistro-strike-modal .sh-btn-close { background:#4a4a5a; color:#ccc; }',
      '#maistro-strike-modal .sh-btn-close:hover { background:#6b6b7b; }',
      '#maistro-strike-modal .sh-locked-msg {',
      ...
      '#maistro-strike-overlay, #tool-modal { touch-action:none; -webkit-overflow-scrolling:touch; }',
      '#maistro-strike-modal, #tool-modal > div { touch-action:auto; }',
  ...
  function showStrikeModal(errorData) {
    ...
    var existing = document.getElementById('maistro-strike-overlay');
    ...
    overlay.id = 'maistro-strike-overlay';

    var modal = document.createElement('div');
    modal.id = 'maistro-strike-modal';
  ...
  function _shLoadDungeonBadge() {
    _origFetch('/v1/maistro/admin/strikes', {headers: authHeaders()}).then(function(resp) {
      if (!resp.ok) return;
      return resp.json();
...
(function() {
  ...
  function showWizard(profile) {
    ...
    var steps = [
      {
        icon: '&#x1F3F0;',
        title: 'Welcome to the Maistro, ' + displayName,
